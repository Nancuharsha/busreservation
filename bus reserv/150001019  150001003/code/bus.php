...
$sql="SELECT via.Dept_time,via.Arr_time,via.fare,C.Bus_type,C.Seats_type,C.B_id,C.O_id,C.R_id,C.O_name FROM via JOIN  (SELECT Buses.Bus_type,Buses.Seats_type,A.B_id,A.O_id,A.R_id,A.O_name FROM Buses JOIN (SELECT Operator.O_name,T.O_id,T.B_id,T.R_id FROM Operator JOIN (SELECT O_id,B_id,R_id FROM via_days WHERE R_id=(SELECT R_id FROM routes where Dept_city='$from' && Arr_city='$to') && Days='$res')T on Operator.O_id=T.O_id)A ON Buses.B_id=A.B_id && Buses.O_id=A.O_id)C on via.O_id=C.O_id && via.R_id=C.R_id && via.B_id=C.B_id";
